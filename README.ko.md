...
> 최종 수정: 2026-09-15
> 터키어 버전: [README.tr.md](README.tr.md) · 상세 가이드: `docs/` (저장소 외부, 로컬 디스크)
...
### Collection 화면 — 최근 추가 (2026-09)

- **플레이어 높이 고정** — 타임라인이 길어져도 영상을 밀어내지 못하며, 긴 목록은 패널 안에서 스크롤됩니다.
- **색상 재생 바** — ▶ ⟲ ⟳ 버튼 옆에 컬렉션에서 *실제 사용된* 색상마다 버튼 하나. 누르면 그 색(= 그 인물)의 모든 카메라 띠가 처음부터 끝까지 순서대로 재생되고 사이 공백은 건너뜁니다. 같은 밴드에서 겹치거나 맞닿은 띠는 하나로 합쳐집니다. 목록이 끝나면 정지. 사용자가 직접 탐색(seek)하면 목록이 취소됩니다. 띠 하나를 클릭하면 그 구간만 재생.
- **모두 펼치기 / 접기** 버튼 — hover 로 밴드 하나만 펼치던 것의 고정형; 두 밴드를 나란히 비교할 때 사용. 휠 = 시간축 확대, Ctrl+휠 = 목록 스크롤.
- **드래그 미리보기** — 색이 있는 띠를 색이 없는 띠 위로 끌 때 대상 박스와 크롭 테두리가 원본 색을 띱니다(연결하면 어떻게 될지 미리 표시).
